...
function buildGamePage(gameName, coupons, imageUrl, genre) {
  ...
  const couponRows = coupons.map(c => `
    <div class="coupon-row">
      ...
      <div class="source-text">출처: ${c.source || '블루스택'}</div>
    </div>`).join('');
...
.logo{font-size:18px;font-weight:800;color:#efefef}
...
.game-icon{width:72px;height:72px;border-radius:14px;object-fit:cover;background:#1a1a2c;flex-shrink:0}
...
.copy-btn.done{background:#3ecf8e;color:#000}
...
.back-link{display:inline-flex;align-items:center;gap:6px;color:#aaa;font-size:13px;margin-bottom:1.5rem;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);padding:7px 14px;border-radius:8px}
.back-link:hover{color:#efefef}
...
<div class="container">
  ...
  <div class="game-header">
    ${imageUrl ? `<img class="game-icon" src="${imageUrl}" alt="${gameName} 아이콘">` : `<div class="game-icon" style="display:flex;align-items:center;justify-content:center;font-size:32px">🎮</div>`}
    <div>
  ...
  ${couponCount > 0 ? couponRows : '<div class="empty">현재 등록된 쿠폰이 없어요.<br>새 쿠폰이 추가되면 바로 알려드릴게요!</div>'}
...
async function pushToGitHub(files) {
  ...
  try {
    ...
    ['game', 'guide', 'news'].forEach(dir => {
      if (!fs.existsSync(`/tmp/site/${dir}`)) {
        fs.mkdirSync(`/tmp/site/${dir}`, { recursive: true });
      }
    });
  ...
  try {
    ...
    snap.forEach(doc => {
      ...
      if (d.expire && d.expire !== '무기한') {
        ...
        if ((today - new Date(d.createdAt)) / 86400000 > 30) { batch.delete(doc.ref); cnt++; }
      }
    });
...
async function main() {
  ...
  console.log('🚀 쿠폰던전 자동수집 + SEO 페이지 생성 시작!');
  console.log(`📅 ${new Date().toLocaleString('ko-KR')}\n`);

  await cleanCoupons();
  const gameList = await collectGameList();

  console.log(`\n🎮 게임별 쿠폰 + 페이지 생성 시작 (${gameList.length}개)\n`);

  const allCoupons = [];
  const gamePageFiles = {}; // { 'game/원신.html': html내용 }
  const gamePageSlugs = []; // sitemap용

  // Firebase에서 공략/뉴스 목록 가져오기 (sitemap용)
  ...
  for (const game of gameList) {
    try {
      ...
      // SEO 게임 페이지 생성 (쿠폰 없어도 페이지는 만듦)
      const slug = game.slug || toSlug(gameName);
      if (slug) {
        gamePageFiles[`game/${slug}.html`] = buildGamePage(gameName, validCoupons, imageUrl, genre);
        gamePageSlugs.push(slug);
      }

      const status = validCoupons.length > 0 ? `✅ ${validCoupons.length}개` : '⬜ 쿠폰 없음';
      console.log(`  ${status} | ${gameName} | 이미지: ${imageUrl ? '✓' : '✗'}`);
  ...
  console.log(`  SEO 페이지 생성: ${gamePageSlugs.length}개`);
  console.log('══════════════════════════════════════');
}
